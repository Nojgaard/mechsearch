...
class Vertex:
    def __init__(self, original_vertex: mod.GraphVertex):
        self._original: mod.GraphVertex = original_vertex
...
class RuleVertex:
    def __init__(self, original_vertex: mod.RuleVertex):
        self._original: mod.RuleVertex = original_vertex
    ...
    def record_edge(self, edge: mod.RuleEdge):
        if edge.left:
            self.record_left_edge(edge.left)
        if edge.right:
            self.record_right_edge(edge.right)

    def record_left_edge(self, edge: mod.RuleLeftGraphEdge):
    ...
    def record_right_edge(self, edge: mod.RuleRightGraphEdge):
        if edge.bondType == mod.BondType.Single:
            self._right_bound_electrons += 1
        elif edge.bondType == mod.BondType.Double:
...
def abstract_graph(graph: mod.Graph) -> mod.Graph:
    new_nodes: Dict[mod.GraphVertex, Vertex] = {}
    preserved_edges: List[mod.GraphEdge] = list()

    for edge in graph.edges:
        preserved_nodes: Set[mod.GraphVertex] = set()
        hydrogenated_node: Optional[mod.GraphVertex] = None
...
def abstract_rule(rule: mod.Rule) -> mod.Rule:
    new_nodes: Dict[mod.RuleVertex, RuleVertex] = dict()
    preserved_edges: List[mod.RuleEdge] = list()

    for edge in rule.edges:
        preserved_nodes: Set[mod.RuleVertex] = set()
        hydrogen_node: Optional[mod.RuleVertex] = None
